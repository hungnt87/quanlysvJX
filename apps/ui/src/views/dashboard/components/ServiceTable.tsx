...
export function ServiceTable({ services, selected, onSelect, onAction }: Props) {
  const isS3RelayRunning = React.useMemo(() => services.some(
    (s) => s.name === 's3relay' && (s.state === 'running' || s.state === 'starting')
  ), [services]);
  
  const areOtherServicesRunning = React.useMemo(() => services.some(
        ...
        <Table.Tbody>
          {services.map((service) => (
            <ServiceRow
              ...
              onSelect={onSelect}
              onAction={onAction}
              isS3RelayRunning={isS3RelayRunning}
